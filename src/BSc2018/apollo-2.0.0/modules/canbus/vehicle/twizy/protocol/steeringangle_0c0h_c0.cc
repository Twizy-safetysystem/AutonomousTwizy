...
void Steeringangle0c0hc0::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_steering_angle()->set_sas_chcksum(sas_chcksum(bytes, length));
  chassis->mutable_steering_angle()->set_msg_count(msg_count(bytes, length));
  chassis->mutable_steering_angle()->set_lws_stat(lws_stat(bytes, length));
  chassis->mutable_steering_angle()->set_lws_speed(lws_speed(bytes, length));
  chassis->mutable_steering_angle()->set_lws(lws(bytes, length));
}
